...
drop table if exists publisher_provider_quality_sum_temp;
create table publisher_provider_quality_sum_temp as
...
from ay_temp_pq a
        left outer join client_portal.contents ct
        on (a.rg_guid = ct.uuid) 
...
group by date(from_utc_timestamp(cast(time_local as bigint),'UTC')),
        a.y, a.m, a.d, 
        reflect('java.net.URLDecoder', 'decode', a.rg_publisher, 'utf-8'),
        ct.company_id,
        case when COALESCE(ci.note, c.note, i.note) is not NULL then 1 else 0 end
;

drop table if exists publisher_provider_quality_summary_temp;
create table publisher_provider_quality_summary_temp as
select a.publisher,
        coalesce(a.company_id, 0) company_id,
	...
	adError,
	b.company
from publisher_provider_quality_sum_temp a left join company_site_map b
	on (a.publisher = b.rg_publisher);
	
insert into table publisher_provider_quality_summary
select publisher,
        company_id,
        event_date, y, m, d,
	is_bot,
	...
	adError,
	company
from publisher_provider_quality_summary_temp a
;