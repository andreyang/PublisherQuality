...
from (
	...
	sum(is_bot*ad_ct) bot_ad_ct, sum(ad_ct) ad_ct,
	sum(is_bot*stream_ct) bot_stream_ct, sum(stream_ct) stream_ct
	from publisher_provider_quality_summary
	where event_date >= str_to_date(concat(year(subdate(curdate(), INTERVAL 1 day)), '-', month(subdate(curdate(), INTERVAL 1 day)), '-01'),'%Y-%m-%d')
	and event_date < curdate()
...
select max(company) company,
	...
	c.name provider,
        ...
        sum(adError) adError
        from publisher_provider_quality_summary a left join client_portal.companies c on (a.company_id = c.id)
        where event_date >= str_to_date(concat(year(subdate(curdate(), INTERVAL 1 day)), '-', month(subdate(curdate(), INTERVAL 1 day)), '-01'),'%Y-%m-%d')
        and event_date < curdate()
	group by publisher, c.name
	union all
select 	NULL company,
	...
	c.name provider,
        ...
        sum(adsStopped) adsStopped,
        sum(adError) adError
        from publisher_provider_quality_summary_butterfly a join client_portal.domains d on (a.domain_id = id)
        join client_portal.publishers p on (d.publisher_id = p.id)
        left join client_portal.companies c on (a.company_id = c.id)
        where event_date >= str_to_date(concat(year(subdate(curdate(), INTERVAL 1 day)), '-', month(subdate(curdate(), INTERVAL 1 day)), '-01'),'%Y-%m-%d')
        and event_date < curdate()
	group by lower(p.name), c.name
        ) a
        left outer join
        (select publisher,
        sum(ip_ct) ip_ct,
...
use warehouse_stg;
select 'Overall filter rate:' ayayayayay from publisher_provider_quality_summary
limit 1
;
